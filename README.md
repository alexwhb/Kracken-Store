...
inspired and adjusted from [ReKamp](https://github.com/xorum-io/ReKamp) to Kotlin Multiplatform, which corresponds to [ReKotlin/1.0.4](https://github.com/ReKotlin/ReKotlin/releases/tag/1.0.4). Supports **JVM**, **Android**, **iOS**.

## Introduction

This takes a lot from [ReKamp](https://github.com/xorum-io/ReKamp), but adds better support for Multithreading on Native platfroms, which was really important for our
application. We use a AtomicRef for our store data, so we can access it easily from other threads on native platforms. This project
Will likely continue to evolve, so stay tuned. 
